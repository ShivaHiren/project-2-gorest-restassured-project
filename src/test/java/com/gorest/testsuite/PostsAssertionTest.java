...
public class PostsAssertionTest extends TestBase {
    ...
    @Test
    public void test002() {
        response.body("find{it.id == 2613}.title", equalTo("Demens sufficio vito vilitas agnitio cultura caterva commodi eos cariosus torqueo dolorum thymbra cado demergo vallum aut culpo."));
    }

    //3. Check the single user_id in the Array list (5522)
    @Test
    public void test003() {
        response.body("[8].user_id", equalTo(5337));
    }

    //4. Check the multiple ids in the ArrayList (2693, 2684,2681)
    @Test
    public void test004() {
        response.body("id", hasItems(2644, 2645, 2622));
    }
...
//    adflicto. Assentator umquam pel."”
    @Test
    public void test005() {
        response.body("find{it.user_id == 5347}.body", equalTo("Vicissitudo voluptatibus universe. Veniam cariosus tener. Cicuta ait sono. Harum itaque textus. Coadunatio conscendo totus. Quo surgo argentum. Umquam cotidie addo. Aliqua laudantium talis. Conatus copia ut. Utique certus testimonium. Cultellus confido unus. Summopere temptatio supplanto. Crudelis minima adficio. Calcar suppellex canonicus. Tredecim tondeo autem. Torqueo libero tabella."));
    }
}
